...
JNIEXPORT void JNICALL Java_com_statnlp_hybridnetworks_optimizer_set_1gradients(JNIEnv *env, jobject obj, jdoubleArray jdA){
    //jclass jcClass = (*environment)->GetObjectClass(environment,caller);    
    //jfieldID g_Id = (*environment)->GetFieldID(environment,jcClass, "g", "[D");
    //jdoubleArray g_j = (*environment)->GetObjectField (environment,caller, g_Id);    
...
JNIEXPORT void JNICALL Java_com_statnlp_hybridnetworks_optimizer_set_1weights(JNIEnv *env, jobject obj, jdoubleArray jdA){
	 w = (*env)->GetDoubleArrayElements(env,jdA, NULL);
}

JNIEXPORT jdoubleArray JNICALL Java_com_statnlp_hybridnetworks_optimizer_get_1weights(JNIEnv *env, jobject obj) {
