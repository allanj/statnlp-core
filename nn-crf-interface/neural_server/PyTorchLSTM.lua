...
local PyTorchLSTM, parent = torch.class('PyTorchLSTM', 'nn.AbstractRecurrent')

function PyTorchLSTM:__init(inputSize, outputSize, rho, cell2gate, p, mono)
...
-------------------------- factory methods -----------------------------
function PyTorchLSTM:buildGate()
...
function PyTorchLSTM:buildInputGate()
   self.inputGate = self:buildGate()
   return self.inputGate
end

function PyTorchLSTM:buildForgetGate()
   self.forgetGate = self:buildGate()
   return self.forgetGate
end

function PyTorchLSTM:buildHidden()
...
function PyTorchLSTM:buildCell()
   -- build
...
function PyTorchLSTM:buildOutputGate()
...
function PyTorchLSTM:buildModel()
   -- build components
...
function PyTorchLSTM:getHiddenState(step, input)
...
function PyTorchLSTM:setHiddenState(step, hiddenState)
   step = step == nil and (self.step - 1) or (step < 0) and (self.step - step - 1) or step
...
------------------------- forward backward -----------------------------
function PyTorchLSTM:updateOutput(input)
...
function PyTorchLSTM:getGradHiddenState(step)
...
function PyTorchLSTM:setGradHiddenState(step, gradHiddenState)
   local _step = self.updateGradInputStep or self.step
   step = step == nil and (_step - 1) or (step < 0) and (_step - step - 1) or step
...
function PyTorchLSTM:_updateGradInput(input, gradOutput)
...
function PyTorchLSTM:_accGradParameters(input, gradOutput, scale)
...
function PyTorchLSTM:clearState()
   self.zeroTensor:set()
...
function PyTorchLSTM:type(type, ...)
   if type then
      self:forget()
      self:clearState()
